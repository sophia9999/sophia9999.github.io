...
## ❓ 이 글이 async/await을 다루는 방식은 조금 다릅니다

많은 개발자들이 Python의 `async/await`을 **단순한 비동기 문법**,  
혹은 **I/O 바운드 작업을 더 효율적으로 처리하는 방식** 정도로 받아들일 것으로 예상됩니다. 

저 역시 처음엔 그렇게 받아들였지만,(혹시 저만 그랬을 까요..👀)\
어느 순간 이런 생각이 들었습니다:

> "이 흐름은 운영체제 위에서 어떤 식으로 작동하는 걸까?"

이 글은 그 질문에서 출발해  
Python의 `async/await` 구조를 **운영체제 관점**에서 차근차근 풀어보려는 시도입니다.

> `async/await`이 **운영체제 위에서 어떤 흐름 구조로 작동하는지**,  
> 특히 **CPU, 프로세스, 스레드, 타임슬라이스**와 어떤 관계가 있는지를  
> **운영체제 관점에서 해석**하려는 시도입니다.
...
### 그래서 이 글의 시선은 여기로 향합니다:

> **"코드의 흐름이 운영체제 위에서 어떤 방식으로 구현되고 실행되는가"**

즉, 이 글은 `async/await`을 단순히 사용하는 방법이 아니라  
**Python 런타임이 사용자 코드의 흐름을 어떻게 제어하고**,  
그 흐름이 **운영체제의 스케줄링과 어떻게 다르게 작동하는지를**  
**논리적으로 해부**합니다.
...
결국엔 "내 프로그램이 실제로 **CPU 위에서 어떤 식으로 실행되고 있는가?**"라는  
근본적인 질문으로 이어집니다.
...
- `async`는 싱글 스레드에서 동시성을 제공한다고 하는데,  
  → **동시성은 "진짜 동시에"가 아니라**,  
